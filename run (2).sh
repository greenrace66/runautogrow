...
# 6. Extract MGLTools
tar -xvf mgltools_x86_64Linux2_1.5.6p1.tar.gz
...
for file in *.json; do
  ...
  jq ".LipinskiLenientFilter = true" "$file" > temp.json && mv temp.json "$file"
  ...
  jq ".dock_choice = \"QuickVina2Docking\"" "$file" > temp.json && mv temp.json "$file"
